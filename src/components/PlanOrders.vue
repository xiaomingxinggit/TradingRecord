...
import {
  ElAlert, ElButton, ElCard, ElEmpty, ElForm, ElFormItem, ElImage, ElInput,
...
let generation = 0, readController: AbortController | undefined, ocrController: AbortController | undefined, writeController: AbortController | undefined
const current = computed(() => drafts.value.find(row => row.id === currentId.value))
const dirty = computed(() => drafts.value.length > 0 || !!preview.value)
const busy = computed(() => loading.value || recognizing.value || saving.value || !!changingId.value)
...
function pasteImage(event: ClipboardEvent) {
  ...
  if (files.length !== 1) { error.value = '每次只能粘贴一张订单截图。'; return }
...
async function confirmDiscard() {
  ...
  try {
    ...
    drafts.value = []; currentId.value = ''; resetPreview(); error.value = ''
    return true
  } catch { return false }
}

watch(() => props.planId, () => { generation++; readController?.abort(); ocrController?.abort(); writeController?.abort(); drafts.value = []; currentId.value = ''; resetPreview(); void load() }, { immediate: true })
window.addEventListener('paste', pasteImage, true)
onBeforeUnmount(() => { generation++; readController?.abort(); ocrController?.abort(); writeController?.abort(); resetPreview(); window.removeEventListener('paste', pasteImage, true); emit('stateChange', { dirty: false, busy: false }) })
...
<style scoped>
.linked-orders{display:grid;gap:18px}.order-help{font-size:12px;line-height:1.8;color:var(--el-text-color-secondary);overflow-wrap:anywhere}.order-actions{display:flex;gap:10px;flex-wrap:wrap}.order-actions .el-button{margin:0}.order-preview{width:100%;max-height:320px;margin:16px 0;background:var(--el-fill-color-light);border-radius:8px}.order-warning{margin-top:10px}.order-drafts{margin-top:18px;border-top:1px solid var(--el-border-color-lighter);padding-top:18px}.order-draft-form{margin-top:20px}.order-fields{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0 16px;margin-top:14px}.order-fields-primary{grid-template-columns:repeat(5,minmax(0,1fr))}.order-fields .el-input-number{width:100%}.order-raw{white-space:pre-wrap;overflow-wrap:anywhere;color:var(--el-text-color-secondary);font-size:12px}.order-list-card :deep(.el-card__body){padding:0}.order-list-card :deep(.el-empty){padding:34px}.order-status-tag{cursor:pointer}.order-list-table{--el-table-header-bg-color:var(--plan-table-bg,#fcfbfe)}summary{cursor:pointer;color:var(--el-color-primary);font-size:13px}@media(max-width:900px){.order-fields-primary,.order-fields{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:560px){.order-fields-primary,.order-fields{grid-template-columns:1fr}.plan-card-heading{align-items:flex-start}.order-actions{width:100%}}
</style>
