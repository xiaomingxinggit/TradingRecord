...
import { ChevronDown, ImagePlus, Plus, ReceiptText } from 'lucide-vue-next'
  ...
  <div class="linked-orders">
    ...
    <ElCard shadow="never" class="order-list-card">
      ...
      <ElTable v-else-if="orders.length" :data="orders" size="small" class="order-list-table">
        <ElTableColumn label="订单号" min-width="178"><template #default="{ row }"><div class="saved-order-ticket"><span><ReceiptText :size="15"/></span><strong>{{ row.ticket }}</strong></div></template></ElTableColumn>
        <ElTableColumn label="品种" min-width="112"><template #default="{ row }"><ElTag size="small" type="info" effect="plain" class="order-symbol-tag">{{ row.symbol }}</ElTag></template></ElTableColumn>
        <ElTableColumn label="方向" width="88"><template #default="{ row }"><ElTag size="small" :type="row.side === 'buy' ? 'success' : 'danger'" effect="light" class="order-side-tag">{{ sideLabel(row.side) }}</ElTag></template></ElTableColumn>
        <ElTableColumn label="手数" width="92" align="right"><template #default="{ row }"><span class="order-number" :class="{ muted: row.volume === null }">{{ row.volume ?? '—' }}</span></template></ElTableColumn>
        <ElTableColumn label="开仓价" min-width="116" align="right"><template #default="{ row }"><span class="order-number" :class="{ muted: row.openPrice === null }">{{ row.openPrice ?? '—' }}</span></template></ElTableColumn>
        <ElTableColumn label="止损" min-width="116" align="right"><template #default="{ row }"><span class="order-number order-stop" :class="{ muted: row.reportedSL === null }">{{ row.reportedSL ?? '—' }}</span></template></ElTableColumn>
        <ElTableColumn label="止盈" min-width="116" align="right"><template #default="{ row }"><span class="order-number order-target" :class="{ muted: row.reportedTP === null }">{{ row.reportedTP ?? '—' }}</span></template></ElTableColumn>
        <ElTableColumn label="当前状态" width="126"><template #default="{ row }"><ElPopover trigger="click" placement="bottom" :width="250" :disabled="locked"><template #reference><ElTag class="order-status-tag" effect="plain" :type="row.status === 'closed' ? 'success' : row.status === 'open' ? 'warning' : 'info'" tabindex="0" :aria-label="`修改订单 ${row.ticket} 状态`">{{ orderStatusLabels[row.status as OrderStatus] }}<ChevronDown :size="13"/></ElTag></template><ElRadioGroup :model-value="row.status" size="small" :disabled="locked || changingId === row.id" @change="value => changeStatusFromTable(row, value)"><ElRadioButton v-for="(label, value) in orderStatusLabels" :key="value" :value="value">{{ label }}</ElRadioButton></ElRadioGroup></ElPopover></template></ElTableColumn>
        <ElTableColumn label="更新时间" min-width="178"><template #default="{ row }"><span class="order-updated-at">{{ timestamp(row.updatedAt) }}</span></template></ElTableColumn>
      </ElTable>
      <ElEmpty v-else description="当前计划还没有关联订单"/>
    </ElCard>

    <ElDialog v-model="compareDialog" class="order-compare-dialog" title="对比订单截图" width="min(720px, calc(100vw - 32px))" :before-close="closeComparison" :close-on-click-modal="false" :close-on-press-escape="!busy">
