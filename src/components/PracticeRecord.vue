...
import { ElAlert, ElButton, ElCard, ElDescriptions, ElDescriptionsItem, ElForm, ElFormItem, ElImage, ElInput, ElInputNumber, ElMessage, ElMessageBox, ElOption, ElRadioButton, ElRadioGroup, ElSelect, ElSkeleton, ElTag, genFileId } from 'element-plus'
  ...
  <section class="practice-record page-stack">
    ...
    <ElCard v-else-if="editing" shadow="never">
      <ElForm label-position="top" :disabled="busy" @submit.prevent="saveRecord">
        ...
        <p class="practice-hint practice-form-note">草稿可先保存不完整内容。模拟持仓中和已平仓只需填写品种、方向与分析周期。</p>
        ...
        <div class="practice-save-bar"><span class="practice-hint">保存分析、截图和所选状态，稍后可继续复盘。</span><div><ElButton v-if="record" :disabled="busy" @click="cancelEdit">取消编辑</ElButton><ElButton type="primary" native-type="submit" :loading="saving" :disabled="busy || conflict"><Save :size="15"/>保存{{ practiceStatus(form.status) }}</ElButton></div></div>
      </ElForm>
    </ElCard>
    <template v-else-if="record">
      <ElCard shadow="never"><template #header><div class="practice-section-head"><h2>{{ record.symbol || '未填品种' }} · {{ practiceSide(record.side) }}</h2><ElTag :type="record.status === 'closed' ? 'success' : record.status === 'open' ? 'warning' : 'info'">{{ practiceStatus(record.status) }}</ElTag></div></template>
        ...
        <p class="practice-hint practice-form-note">开平仓信息暂不采集，此处仅保留历史数据。只有已平仓且有明确净盈亏和币种的历史记录参与盈亏统计；缺失结果不当作持平。</p>
