    ...
    <ElCollapse v-model="sections">
      ...
      <ElCollapseItem title="关联订单" name="orders">
        <div class="page-stack">
          ...
          <ElText>盈亏比按当前开仓价、截图止损和截图止盈的价格距离计算收益 / 风险，不计交易成本，不代表实际盈利。做多须为止损 &lt; 开仓价 &lt; 止盈，做空须为止盈 &lt; 开仓价 &lt; 止损；缺少字段或关系无效时显示“—”，导出采用相同口径。</ElText>
          <ElText>已平仓订单可点击“锁定订单”并再次确认；锁定后订单视为完结，无法再修改状态、手数、止损和止盈，不能解锁。锁定时间和变化会被记录，仍可查看和导出；未平仓订单显示“待平仓”。</ElText>
          <ElText>“更新订单”只比较当前计划同一订单号的手数、止损和止盈，已锁定订单不能更新。未知字段不清空旧值，确认后才保存；对比期间的选择或粘贴图片不会进入新订单导入。未保存的导入或对比草稿离开前会提示。</ElText>
        </div>
      </ElCollapseItem>
