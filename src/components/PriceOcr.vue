...
const props = withDefaults(defineProps<{ disabled?: boolean; targetLabel?: string }>(), { targetLabel: '原计划价位' })
const emit = defineEmits<{ apply: [row: PriceRow]; 'active-change': [active: boolean] }>()
...
function apply(row: PriceRow) {
  ...
  ElMessage.success('价位已填入所选草稿，请核对后保存；未设置的止损、止盈已留空。')
}
function openDialog() { if (!props.disabled) { reset(); open.value = true } }
watch(open, active => { emit('active-change', active); if (!active) reset() }, { flush: 'sync' })
watch(() => props.disabled, disabled => { if (disabled && open.value) open.value = false })
async function recognize(file?: File) {
  if (!file || busy.value || !open.value || props.disabled) return
  reset()
  ...
  try {
    ...
    const response = await fetch('/api/price-ocr', { method: 'POST', body, signal: requestController.signal })
...
onBeforeUnmount(() => { reset(); emit('active-change', false); window.removeEventListener('paste', paste, true) })
</script>

<template>
  <ElButton native-type="button" :disabled="disabled" size="small" @click="openDialog"><ScanLine :size="14"/>图片识别</ElButton>
  <ElDialog v-model="open" title="识别持仓 / 挂单价位" width="min(760px, 94vw)" append-to-body @submit.prevent.stop>
    <ElAlert :title="`填入目标：${targetLabel}`" type="info" :closable="false" class="price-ocr-target"/>
    ...
    <ElButton native-type="button" :loading="busy" @click="input?.click()">{{ busy ? '正在识别…' : '选择截图' }}</ElButton>
    ...
    <ElTable v-if="rows.length > 1" :data="rows">
      ...
      <ElTableColumn label="操作"><template #default="{ $index }"><ElButton native-type="button" link type="primary" @click="apply(rows[$index]!)">填入此行</ElButton></template></ElTableColumn>
    </ElTable>
  </ElDialog>
</template>

<style scoped>
.price-ocr-help { margin: 0 0 16px; line-height: 1.7; color: var(--el-text-color-secondary); }
.price-ocr-target { margin-bottom: 16px; overflow-wrap: anywhere; }
button + .price-ocr-help { margin-top: 16px; }
.price-ocr-preview { display: block; width: 100%; margin: 16px 0; border: 1px solid var(--el-border-color-light); }
</style>
