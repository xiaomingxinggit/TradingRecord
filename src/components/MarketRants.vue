...
import { ElAlert, ElButton, ElCard, ElEmpty, ElForm, ElFormItem, ElImage, ElImageViewer, ElInput, ElMessage, ElMessageBox, ElPagination, ElSkeleton } from 'element-plus'
...
async function load(requestedPage = page.value) {
  if (loading.value || publishing.value || savingReplyId.value) return
  loading.value = true
  loadError.value = ''
...
async function publishReply(rantId: string) {
  ...
  try {
    ...
    if (rant) {
      rant.replies.push(result.reply)
  ...
  } finally { if (mounted) savingReplyId.value = '' }
}
async function confirmDiscard() {
  if (publishing.value || savingReplyId.value) { ElMessage.warning('内容正在发布，请完成后再切换。'); return false }
  if (confirming.value) return false
  if (!dirty.value) return true
  confirming.value = true
  try {
    await ElMessageBox.confirm('尚有未发布的吐槽或回复，离开将丢失这些草稿。', '离开行情吐槽', {
...
function beforeUnload(event: BeforeUnloadEvent) {
  if (dirty.value || publishing.value || savingReplyId.value) { event.preventDefault(); event.returnValue = '' }
}
onMounted(() => {
  void load()
...
<template>
  <section class="rants-page">
    ...
    <section class="rant-feed" aria-label="行情吐槽时间线" :aria-busy="loading">
      <div class="feed-heading"><div><h2>行情时间线</h2><span>{{ loaded ? `共 ${total} 条记录 · 最新在前` : '按发布时间倒序' }}</span></div><ElButton :loading="loading" :disabled="disabled" @click="load(page)"><RefreshCw v-if="!loading" :size="15"/>刷新时间线</ElButton></div>
      ...
      <ol v-if="rants.length" class="rant-timeline">
        <li v-for="rant in rants" :key="rant.id" class="rant-item">
          ...
          <article class="rant-entry">
            ...
            <section class="reply-section" :aria-label="`回复 ${dateTime(rant.createdAt)} 的行情随记`">
              <h3>回复 {{ rant.replyCount }}</h3>
      ...
      <ElPagination v-if="loaded && total > pageSize" class="feed-pagination" background layout="prev, pager, next" :current-page="page" :page-size="pageSize" :total="total" :disabled="disabled" @current-change="load"/>
    </section>
